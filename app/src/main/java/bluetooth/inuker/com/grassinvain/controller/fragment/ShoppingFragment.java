...
public class ShoppingFragment extends Fragment implements ShoppingCartListenter, View.OnClickListener {
    ...
    private void initRecyclerView() {
        ...
        shopCartAdapter.setResfreshListener(new ShopCart2Adapter.OnResfreshListener() {
            ...
            public void onResfresh(boolean isSelect) {
                ...
                shoppingCart_zongjia.setText(TextUtil.fen2yuan((int)mTotalPrice1));
            ...
            case R.id.shoppingCart_jiesuan:
                if (shoppingCart_jiesuan.getText().toString().trim().equals("删除")) {
                    ...
                    if (0 != productSDeatilBodyList.size()) {
                        ...
                        shopCartAdapter.notifyDataSetChanged();
                        userModel.getdeleteProduct(deleteList, new Callback<Object>() {
                            @Override
    ...
    private void digui() {
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i).isCheckout()) {
                deleteList.add(Long.parseLong(list.get(i).shopCarId));
                list.remove(i);
                digui();
            }
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if(resultCode==5){
            Toast.makeText(getActivity(),"55555555",Toast.LENGTH_SHORT).show();
        }

        if(resultCode==10){
            Toast.makeText(getActivity(),"100000",Toast.LENGTH_SHORT).show();
        }
